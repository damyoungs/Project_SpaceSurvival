 ...
 *       
 *       
 *       적이 죽으면 신호를 어디로 쏴줄것인가    Enemy(죽을때 자기 자신을 파라미터로  ItemSpawner의 SpawnItem 함수 호출,  드랍아이템 Table(Dictionary)확률 리스트) ->  
 *       case 적 -> 스포너 확률적으로 드롭 -> 타일 -> 타일에서 드롭이 됐는지 확인 만약 있다면  -> 플레이어가 타일 위로 이동시 아이템 습득
 *       이렇게 되면 굳이 아이템스포너가 굳이 타일에 신호를 쏴줄 이유가 있는가? 신호를 쏴주는 이유가 아이템이 소환됐다는 것을 알기 위함인데 타일 내부에서 현재 타일위에 무엇이있는지 체킹하고있기때문에
 *       굳이 따로 신호를 보낼필요가 있을까?
 *       
