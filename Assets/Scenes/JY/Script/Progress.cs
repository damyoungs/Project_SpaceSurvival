 ...
 *       *  할일목록 
 *        
 *       slotManager의  ItemSwap함수에서  이미지를 바꾸는 함수를 만들고 그 후 아이템의 정보를 슬롯에 업데이트 시켜주려했는데 그냥 해당슬롯을 지우고 다시 만든 뒤 처음 클릭했던 슬롯과 두번째 클릭한 슬롯을 파라미터로 
 *       넘겨서 지우고 위치만 바꿔 새로 만들어 리스트에 업데이트 시켜야겠다는 생각이 든다 왜냐하면 나중에 soting기능을 추가하려면 어차피 슬롯을 지우고 다시 생성하는 기능이 필요하다고 생각했다
 *       생각해보니 sorting기능을 만들려면 
 *       어차피 
 *      
 *       slot오브젝트가 움직일때 하위 Image 오브젝트의 로컬포지션이 같이 변경되는 문제가 있다
 *      
 *      
 *      
 *      
