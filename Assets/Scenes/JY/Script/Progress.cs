...
인벤토리 시스템 구현:
    ...
    enum : 장비, 소비, 재료, 기타
        ...
        재료 : 
                        ...
                        계층구조
                        GameManager(SingleTon) - Inventory, UI_Spawner, Item_Spawner, ButtonController 등 기능별 중심 클래스의 참조를 참조해서 다른곳에서 필요할때 찾아 쓸수있도록 한다.
 ...
 * 
 *      UI버튼을 함수에 직접 연결해주는데 연결된 함구의 위치가 제각각 모두 다른클레스안에 위치한다면 나중에 버튼이 많아질때 찾는 시간이 오래걸리고 불편하다. 따라서 버튼기능을 모아놓은 클레스가 필요할것같다
 *  
 ...
 *      Pool생성 Dictionary<int, Queue<GameObject>> /완료
 *      
 *      SetActive false 할때 자동으로 returnPool, enQueue 시키기
 *      - Item오브젝트 만들기 포션류 먼저
 *      Animation/완료
 *      
 *      ItemSpawner에 위치값을 파라미터로 받아서 그곳에 아이템오브젝트 생성하는 함수 작성
 *      
 *      Interface 를 이용해서 Item이 반드시 가져야할 속성을 정의하고 상속, 데이터 작성.
 *      ItemData 와 각 아이템들은 MonoBehaviour 를 상속받지 않아도 될 것 같다 생성자로 초기화를 해야할것같다
 *      
 *      
 *      타일에 트리거 콜라이더 생성 후 플레이어가 이동하면 해당 아이템을 획득
 *      
 *      
 * 
 * 
