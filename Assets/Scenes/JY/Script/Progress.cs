 ...
 *      
 *      6월 27일 FeedBack
 *      buttoncontroller 삭제 재연결, ItemData 인터페이스의 내용을 그냥 ItemBase 안에 모두 포함시키기 굳이 ItemBase를 상속받을건데 ItemData라는 인터페이스를 따로 만들 이유가X
 *      에니메이션 로컬포지션 = 부모오브젝트 생성 후 애니메이터를 부여 후 자식오브젤트의 포지션을 조정한다
 ...
 *      
 *      오브젝트풀, 애니메이션
 *            HpPotion 소환 위치 문제 Localposition, 아이템 인터페이스, 생성위치 - 타일위치? 혹은 적 위치
 *            ButtonController
 *      
 *      
