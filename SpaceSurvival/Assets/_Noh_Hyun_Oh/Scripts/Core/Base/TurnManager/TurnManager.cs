...
public class TurnManager : ChildComponentSingeton<TurnManager>
{
    ...
    /// <summary>
    /// 해당턴에서 삭제된 오브젝트를 턴 게이지 관리자에게 전달할 델리게이트
    /// </summary>
    public Action<ITurnBaseData> removeTurnObjectGauge;

    /// <summary>
    /// 해당턴에서 추가된  오브젝트를 턴 게이지 관리자에게 전달할 델리게이트
    /// </summary>
    ...
    private void TurnStart() {
        ...
        if (turnStartValue < nowTurnUnit.TurnActionValue) //턴진행 할수있는 값이 됬으면 턴진행
        {
    ...
    {
        turnEndObj.TurnEndAction = null; // 턴이꼬이는것을 방지하기위해 턴종료가 끝낫으면 델리게이트를 초기화한다
        turnEndObj.TurnRemove = null; //해당턴이끝낫으니 델리게이트 회수한다.
        turnEndObj.BattleUI.TrunActionStateChange(); //상태이상 턴진행시키기
    ...
    /// <summary>
    /// 턴중간에 몬스터가 죽어서 사라지거나 엔피씨가 사라지는경우 
    /// 턴진행리스트 에서도 사라져야함으로 작성
    /// </summary>
    ...
    public void TurnListDeleteObj(ITurnBaseData deleteObject)
    {
        ...
        {
            deleteObject.ResetData();//초기화 함수 실행
