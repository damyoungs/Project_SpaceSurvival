...
/// </summary>
public class Cho_BattleMapGenerateDoubleArray : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        SpaceSurvival_GameManager.Instance.GetBattleMapTilesDataDoubleArray = () => MapTiles; // 게임메니저에 데이터 저장하기위해 연결 
    }
    ...
    void MapInstantiate()
    {
        ...
        mapTiles = new Tile[sizeY, sizeX];   // 배열 동적 생성
            ...
            for (int x = 0; x < sizeX; x++)
            {
               ...
               (x == sizeX - 1 && y == sizeY - 1)     //우측 최상단 체크
               )
                {
                    ...
                    wallObject = Instantiate(wall, mapTiles[y, x].transform);                      // 측면 벽1 생성
                    wallObject.transform.Translate(new Vector3(1.0f, 0.0f, -1.75f));            // 측면 벽1 이동
                    wallObject = Instantiate(wall, mapTiles[y, x].transform);                      // 측면 벽2 생성
                    ...
                    wallObject = Instantiate(wall, mapTiles[y, x].transform);                      // 꼭지점 벽 생성
                    ...
                    if (x == 0 && y == 0)                                      // 왼쪽 위
                    {
                        mapTiles[y,x].transform.Rotate(new Vector3(0, 90.0f, 0));
                    }
                    else if (x == 0 && y == sizeY - 1)                         // 왼쪽 아래
                    {
                        mapTiles[y,x].transform.Rotate(new Vector3(0, 180.0f, 0));
                    }
                    else if (x == sizeX - 1 && y == 0)                         // 오른쪽 위
                    {
                        mapTiles[y,x].transform.Rotate(new Vector3(0, 0.0f, 0));
                    }
                    else if (x == sizeX - 1 && y == sizeY - 1)                 // 오른쪽 아래
                    {
                        mapTiles[y,x].transform.Rotate(new Vector3(0, 270.0f, 0));
                    }
                }
                     ...
                     )
                {
                    ...
                    wallObject = Instantiate(wall, mapTiles[y,x].transform);                              // 측면 벽 생성
                ...
                else
                {
                    ...
                    mapTiles[y,x].transform.Rotate(new Vector3(0, 90.0f * Random.Range(0, 4), 0));        // 중앙 타일 랜덤 회전(그냥 미관상)-		base	"Custom_Bld_Floor_Small_02(Clone) (Tile)"	UnityEngine.Object

                }
                mapTiles[y,x].transform.position = new Vector3(mainTileSize.x * x, 0, mainTileSize.z * y);
            }
        }
    ...
    {
        mapTiles[y,x] = Instantiate(type, transform).GetComponent<Tile>();     // type에 따른 타일 생성
        mapTiles[y, x].TileType = tileType;                                                // 타일 스크립트에 타입 저장
        mapTiles[y, x].Width = x;                                                      // 타일 가로 인덱스 저정
        mapTiles[y, x].Length = y;                                                    // 타일 세로 인덱스 저정
        mapTiles[y, x].Index = i;
    }
